...
function render(items){const count=s=>items.filter(x=>x.severity===s).length;sevs.forEach(s=>{const b=$(s==='error'?'errors':s==='warning'?'warnings':'infos');b.innerHTML='<span class="dot"></span>'+count(s)+' '+s.charAt(0).toUpperCase()+s.slice(1)+(count(s)===1?'':'s')});const visible=items.filter(x=>activeSevs.has(x.severity));$('findings').innerHTML=visible.map(x=>`<tr class="${esc(x.severity)}"><td>${esc(x.check_id)}</td><td class="sev">${esc(x.severity)}</td><td>${esc(x.file_path)}</td><td>${esc(x.line_number)}</td><td>${esc(x.explanation)}</td><td>${esc(x.fix_suggestion)}</td></tr>`).join('');$('clean').classList.toggle('hidden',items.length>0);$('clean').textContent=items.length>0&&visible.length===0?'No findings match the active filter.':'No security issues found.';$('results').classList.remove('hidden');$('scanner').classList.add('hidden');$('report').disabled=false;$('status').textContent=''}
